...
def validate_live(db,research_dir):
    ...
    return {'captured_at':datetime.now(timezone.utc).isoformat(),'pid':db.fm.pid,'resolution':db.info(),'players':results,'squad':squad_check,'context':context.evidence(),'passed':all(r['passed'] for r in results) and squad_check['passed']}
